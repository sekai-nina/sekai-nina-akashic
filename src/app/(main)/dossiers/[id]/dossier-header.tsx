...
export function DossierHeader({ dossier, editable, manageable }: DossierHeaderProps) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      // 成功すると redirect するので、戻ってくるのは失敗したときだけ
      const res = await deleteDossierAction(dossier.id);
  ...
  return (
    <div className="mt-3 bg-white border border-slate-200 rounded-lg p-5">
      ...
      {deleteError && (
        <p
          role="status"
          className="mt-3 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-md px-3 py-2"
        >
          {deleteError}
        </p>
      )}
