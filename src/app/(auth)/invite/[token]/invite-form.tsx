...
export function InviteForm({ token, email: fixedEmail }: Props) {
  const router = useRouter();
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);

  async function handleDiscordSignup() {
    // Set invite token cookie so the callback can read it
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            readOnly={!!fixedEmail}
            className={`w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500 ${fixedEmail ? "bg-slate-50 text-slate-500" : ""}`}
          />
        </div>
        <div>
