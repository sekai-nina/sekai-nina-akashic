...
| akashic `src/lib/meetgreet/sketch.ts` | OpenAI `gpt-image-2.5-sunburst` | ミーグリのスケッチ生成 | `akashic.meetgreet_sketch` |
...
| `CreditSnapshot` | 観測した残高。`provider` / `observedAt` / `balanceUsd` / `amount` / `currency` / `unitsPerUsd` / `note` |
...
**社内の計算はすべて USD で持つ。** 単価表も各社の公表価格も USD で、OpenAI / Anthropic のコスト API も USD で返すため。ところが **Google Cloud の請求通貨はアカウントによって円**で、AI Studio の残高も円で出る。円で見た額をそのまま `balanceUsd` に入れると残高を 150 倍に見せてしまう。

そこで**目視した金額と通貨を原本として残し**（`amount` / `currency`）、`balanceUsd` には換算値を入れる。レート（`unitsPerUsd` = 1 USD が何単位か）は記録した行に残すので、後からどう換算したかが分かる。

- **為替 API は引かない。** 残高の推定に効くのは数 % で、外部依存を 1 つ増やして取り込みが落ちるほうが痛い。既定値は `src/lib/costs/currency.ts` に置き、ズレていたら記録するときにフォームで上書きする
- レートが 0 以下・異常に大きい値なら**記録を拒否する**（0 除算の Infinity が残高に入ると判定ごと壊れる）
...
- 残高スナップショットの入力フォーム（通貨を選べる。USD 以外はレートも入れる）+ 「今すぐ取り込む」
  - USD 以外で記録した行は、カードに原本（`¥3,000 / 1 USD = 155 JPY`）を添えて出す
