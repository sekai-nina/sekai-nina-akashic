...
**同じ回を二度作らない（#112）。** `(date, format, label)` が同じ行が既にあれば、新しく作らずそれを返す（`200`、作ったときは `201`）。作成は数十秒かかることがあり、タイムアウトして再送するとドシエ・収集が二重にできていた。DB 側にも `@@unique([date, format, label])` があるので、同時に 2 本走っても 1 つに収束する。
...
**削除は REST に無い**（画面だけ）。消せるのは**作った人か管理者**で、ドシエの削除が所有者だけなのに合わせている（#112）。回を消してもドシエと記事そのものは残る。逆に、**ミーグリで使われているドシエは先に回を消さないと削除できない**（スケッチ・切り抜き枠・記事の紐づけ・除外リストが巻き添えで消えるため）。

**レスポンス（作成なら `201`、既にあったものを返すなら `200`）:** 一覧の 1 行と同じ形に `candidates` が付く。
...
### POST /meetgreets/:id/sketch/crops

参照写真の**切り抜き枠**を保存する（#136）。ツーショットの写真をそのまま参照に渡すと隣の人の服を拾うので、本人のところだけを送れるようにする。

```json
{"cmf…abc": {"x": 0.42, "y": 0.05, "w": 0.5, "h": 0.9}, "cmf…def": null}
```

| フィールド | 型 | 説明 |
|---|---|---|
| `<assetId>` | object \| null | 枠、または `null`（枠を外す = 画像全体を使う） |
| `x` / `y` | number (0〜1) | 枠の左上。**画像に対する割合** |
| `w` / `h` | number (0.02〜1) | 枠の幅・高さ（割合） |

**レスポンス:** `{"crops": {"<assetId>": {"x": …, "y": …, "w": …, "h": …}}}`（保存後の全量）。

- **画素ではなく割合で渡す。** 画面で枠を引くのは 640px のサムネイルだが、生成に使うのは Drive の原本を 1280px に縮めたもので解像度が違う
- **ドシエに入っている `internal` 以下の画像に限る**（それ以外は 400）。一度に指定できるのは 60 枚まで
- 枠が画像の外にはみ出すもの、幅か高さが 2% 未満のものは 400
- 枠は回ごとに持つ。同じ画像を別の回で使うときは別の枠になる
- 切り抜いた画像は保存しない。**生成のたびに sharp で切り出す**（アセットもドシエも増えない）。切り出せなかったときは枠を無視して画像全体を送る（参照が 1 枚減るより、隣の人が写っても生成が続くほうがまし）
...
1. event エンティティを決める。`entityId` があればそれ、無ければ**ライブ名と同名の event エンティティを find-or-create**（過去のライブは MV の取り込み等で既にあるので、名前を打ち直して別のエンティティを作らないよう `GET /entities?type=event` で探して渡す）
...
`dossierId` / `repoCollectionId` を渡すと、新しく作らず**既にあるものを使う**（別のライブ / ミーグリに使われているものは 400）。作成は **1 トランザクション**。
...
| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `name` | string (≤120) | ✓ | ライブ名。記事のタイトル（= ファイル名）・ドシエ名・収集名・エンティティ名になる |
| `performances` | array (1〜100) | ✓ | 公演。各要素は `date`（`YYYY-MM-DD`、必須）/ `venue`（≤200）/ `label`（≤50。「昼公演」等）/ `note`（≤500）/ `songs`（string[]。公演限定の追加曲）/ `centerSongs`（string[]） |
| `commonSongs` | string[] (≤100 曲、各 ≤100 文字) | | ライブ共通の披露曲 |
...
- `suggested`（初期チェック）は、本文にライブの話（ライブ / 公演 / ツアー / セトリ 等 + **ライブ名・鍵括弧の中・会場名**）があるブログ、どれかの公演日〜翌日のトーク画像 / 動画、本文にライブの話があるトークのテキスト
- event エンティティ経由のアセット（ライブ MV など）は列挙されるが初期チェックしない
...
- 同じ `id` を 2 回渡すと 400。他のライブの公演の `id` は「無いもの」として扱われ、新しい行が作られる
...
insta-watch が見張るハンドルの一覧。**有効なものだけ**返す。

認証は API キー（読むだけなので read / write の別は問わない）。

```json
{
  "targets": [
    { "handle": "hinatazaka46", "tier": "normal", "intervalMinutes": 18 },
    { "handle": "hiyotan928_official", "tier": "hot", "intervalMinutes": 10 }
  ]
}
```
...
|---|---|
| `handle` | `@` を除いた小文字のハンドル |
| `tier` | `hot` / `normal` / `cold` |
| `intervalMinutes` | 実際に使う間隔（分）。明示指定が無ければ tier の既定（10 / 18 / 240） |
